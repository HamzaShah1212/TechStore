...
export async function GET(request) {
  ...
  // Selected product image, price, and status in query
  const result = await db().execute(`
    SELECT 
      o.*, 
      p.image_url,
      p.price AS product_price,
      GROUP_CONCAT(oi.product_name || ' x' || oi.quantity, ', ') || ' · Phone: ' || o.customer_phone AS items 
    FROM orders o 
...
// Update order status (Pending / Confirm)
...
export async function POST(request) {
  ...
  const order = await db().execute({ sql: "INSERT INTO orders (user_id, customer_name, customer_email, customer_phone, shipping_address, total_amount, status) VALUES (?, ?, ?, ?, ?, ?, 'Pending')", args: [user_id, customer_name, customer_email, customer_phone, shipping_address, Number(item.price) * Number(quantity)] });
